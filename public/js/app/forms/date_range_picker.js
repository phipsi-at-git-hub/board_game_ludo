...
function initDateRangePickers() {
    document
        ...
        .forEach(input => {
            ...
            const localization =
                (input.dataset.uiLocalization || 'en-us').toLowerCase();

            const locale =
                localization.startsWith('de')
                    ? 'de-DE'
                    : 'en-US';

            const withTime =
                input.dataset.uiWithTime === 'true';
            ...
            if (withTime) {
                ...
                const endTimeContainer =
                    document.createElement('div');

                endTimeContainer.className =
                    'date-range-time-group';
                ...
                startTimeContainer.append(
                    startTimeLabel,
                    startTime.container
                );

                endTimeContainer.append(
                    endTimeLabel,
                    endTime.container
                );
            ...
            const existingRange =
                parseInputRange(input.value);

            if (existingRange) {

                rangeStart =
                    cloneDate(existingRange.start);

                rangeEnd =
                    cloneDate(existingRange.end);
                ...
                if (withTime) {
                    ...
                    startTime =
                        `${pad(rangeStart.getHours())}:${pad(rangeStart.getMinutes())}`;

                    endTime =
                        `${pad(rangeEnd.getHours())}:${pad(rangeEnd.getMinutes())}`;
            ...
            function updateTimeState() {
                ...
                startTime =
                    startTimeField.value.trim();

                endTime =
                    endTimeField.value.trim();
            ...
            const applyRange = () => {
                ...
                input.value =
                    `${formatInternalDate(rangeStart)} - ${formatInternalDate(rangeEnd)}`;
                    ...
                    button.addEventListener(
                        ...
                        event => {
                            ...
                            if (withTime) {
                                ...
                                status. textContent = locale === 'de-de' ? 'Start- und Endzeit prüfen oder anpassen' : 'Check or adjust start and end time'; 
    ...
    if (
        !document.body.dataset.dateRangeOutsideClick
    ) {
